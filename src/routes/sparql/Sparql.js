...
class Sparql extends React.Component {
        ...
        for (let j = 0; j < response.data.values[i].length; j++) {
          ...
          if (j == 0) {
            ...
            if (arr!= null && arr.length > 1) {
              ...
              console.log(str);
              if(str.search("jpg")>-1)
              {row = <TableCell align="left" ><a href={`/img/${str}`} target="_blank"><img
  ...
  getInfo = () => {
    var e1 = "SELECT ?s ?p ?o { ?s <rpo:imageof> <http://rich.wangmengsd.com/resource/rps/282> . }";
  ...
  getContent = () => {
    ...
    {return(
      ...
      <div className={s.root}>
      <h1 className={s.title}>Entity Link</h1>
      {this.getUploader()}
      </div>
      </div>
    );}
